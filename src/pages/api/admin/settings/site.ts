...
import { siteConfigSchema } from '../../../../lib/admin/schemas/config';
...
export const PUT: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const parsed = siteConfigSchema.safeParse(body.data);
    if (!parsed.success) {
      return adminError(parsed.error.issues[0]?.message || 'invalid_data', 400);
    }
    const result = await saveSiteConfig(parsed.data, body.sha);
